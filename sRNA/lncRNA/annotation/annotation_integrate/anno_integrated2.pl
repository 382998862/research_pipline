...
# ------------------------------------------------------------------
my ($Anno_dir,$gene,$odir,$out_index,$HELP,$cfg);
...
&USAGE if (!defined $Anno_dir || !defined $odir || !defined $out_index || !defined $cfg) ;
...
if (defined $eggNOG && exists $config{eggNOG}) {
...
if (defined $cog && exists $config{Cog}) {
...
if (defined $kog && exists $config{Kog}) {
...
if (defined $nr && exists $config{nr}) {
...
if (defined $kegg && exists $config{Kegg}) {
...
if (defined $swissprot && exists $config{Swissprot}) {
...
if (defined $trembl && exists $config{TrEMBL}) {
...
if (defined $nt && exists $config{nt}) {
...
sub readcfg{
	...
	while(<IN>){
		chomp;
		next if (/^$|^#|^\s$/);
		my ($key,$info)=split /\s+/,$_,2;
		if(($key eq "nr")||($key eq "Swissprot")||($key eq "Kegg")||($key eq "Pfam")||($key eq "Cog")||($key eq "Kog")||($key eq "eggNOG")||($key eq "nt")||($key eq "TrEMBL")){
			$hash->{$key} = $info;
		}
	}

...
usage <- function() {
	print("-------------------------------------------------------------------------------")
	print("Usage: Rscript phastcons_cdf.r 1.phastcons_score.txt 2.outdir 3.key")
	print("1) phastcons_score: the phasecons score file(should contained mean0 column)")
...
library(ggplot2)
p<-ggplot(data=scores,mapping=aes(x=mean0)) + stat_ecdf(size=1,geom ="smooth")+xlab("PhastCons Score") + ylab("Cumulative Frequency")

...
my ($od,$conf,$count,$filter,$fpkm,$type,$human);
GetOptions(
        "h|?"           =>\&USAGE,
	"od:s"     	=>\$od,
	"count:s"	=>\$count,
...
$filter||="cpm";
$filter=$config{filter}		if(exists $config{filter});
$Com=$config{Method_NR}         if(exists $config{Method_NR});
$Sep=$config{Method_RE}         if(exists $config{Method_RE});

if(defined $PValue && !defined $FDR && $Com eq "EBseq" && exists $config{Com}){
	print "EBseq could not produce PValue! So method for deg group without Biological replicates should be replaced edgeR,please change your config\n";
	die;
}
...
if($Com eq "edgeR"){
	if(defined $human){
		...
		print "if edgeR for no replicates,the parameter \"-human yes\" must be given\n";
		die;
	}
}

my $flag1="";#####contain FPKM info
my $flag2="";
if(defined $fpkm){
	$fpkm=abs_path($fpkm);
	$type||="FPKM";
	if($Com eq "edgeR"){
		print "$Com\n";
		$flag1="--fpkm $fpkm --type $type";
	}else{
		$flag1="$fpkm $type";
	}
	
	if($Sep eq "edgeR"){
		$flag2="--fpkm $fpkm --type $type";
	}else{
		$flag2="$fpkm $type";
	}
}
...
if(@coms>0){
	foreach my $com(@coms){
		my $vs=&getdecfg($com);
		if($Com eq "edgeR"){
			print SH1 "$Rscript $script{$Com} --count $count --config $od/$vs/$vs.de.config --out.prefix $od/$vs/$vs --filter $filter $flag1 --disper $dis && perl $Bin/diff/filter_final_DEG.pl -input $od/$vs/$vs.all -output $od/$vs/$vs.DEG_final.xls $select \n";
		}else{
	                print SH1 "$Rscript $script{$Com} $count $od/$vs/$vs.de.config $od/$vs/$vs $filter $flag1 && perl $Bin/diff/filter_final_DEG.pl -input $od/$vs/$vs.all -output $od/$vs/$vs.DEG_final.xls $select \n";
		}
...
if(@seps>0){
	foreach my $sep(@seps){
		my $vs=&getdecfg($sep);
		if ($Sep eq "edgeR"){
			print SH1 "$Rscript $script{$Sep} --count $count --config $od/$vs/$vs.de.config --out.prefix $od/$vs/$vs --filter $filter $flag2 && perl $Bin/diff/filter_final_DEG.pl -input $od/$vs/$vs.all -output $od/$vs/$vs.DEG_final.xls $select \n";
		}else{
			print SH1 "$Rscript $script{$Sep} $count $od/$vs/$vs.de.config $od/$vs/$vs $filter $flag2 && perl $Bin/diff/filter_final_DEG.pl -input $od/$vs/$vs.all -output $od/$vs/$vs.DEG_final.xls $select \n";
		}
		print SH2 "$Rscript $Bin/draw/pheatmap.r --infile $od/$vs/$vs.DEG_final.xls --outfile $od/$vs/$vs.heatmap --scale none --color.type 1 --legend  --is.log  --show_colnames --cluster_rows && $Rscript $Bin/draw/plot_MA_volcano.r --all $od/$vs/$vs.all $select --od $od/$vs/$vs --type $type\n";
	}
}
...
Example: perl $0 -count All_gene_counts.list -conf DEG_Analysis/gene.cfg -fpkm All_gene_fpkm.list -type FPKM -filter cpm -human yes -od DEG_Analysis/gene

...
# GetOptions
# ------------------------------------------------------------------
my ($idir,$odir,$cfg,$l2mc,$l2mt,$mi2l,$mi2c,$mi2g,$chostgene);
...
foreach my $lnc (keys %h){
	...
	if(scalar(@D)>=1){push @list1,[@D];push @list2,[@D];push @name1,"DE_miRNA_TargetLncRNA";push @name2,"DE_miRNA_TargetLncRNA";}
#	my @list1 = ("@A","@B","@D");
#	my @list2 = ("@A","@C","@D");
...
foreach my $g (keys %h){
	...
	if(scalar(@D)>=1){push @list1,[@D];push @list2,[@D];push @name1,"DE_miRNA_TargetmRNA";push @name2,"DE_miRNA_TargetmRNA"; }
...
#        my @name1 = ("DE_miRNA","DE_mRNA_TargetmiRNA","DE_LncRNA_TargetmiRNA","DE_circRNA_TargetmiRNA");
	...
	if(scalar(@C)>=1){push @list1,[@C];push @name1,"DE_LncRNA_TargetmiRNA";}

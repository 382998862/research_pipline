...
# GetOptions
# ------------------------------------------------------------------
my ($idir,$odir,$data_cfg,$detail_cfg,$l2mc,$l2mt,$mi2l,$mi2c,$mi2g,$chostgene,$l2m,$nl);
...
&USAGE unless ($idir and $odir and $data_cfg and $detail_cfg and $nl);
# ------------------------------------------------------------------
...
if((defined $l2m) && (-e "$l2m")){
	foreach my $test (keys %h){
		...
		while(<IN>){
			chomp;
			if(/^#/){my($id,$info)=split /\t/,$_,2;print OUT "$id\tSymbol\t$info\tdiff_target_gene\tSymbol\tdiff_target_miRNA\n";next;}
			my @A = split /\t/,$_;
			my ($n,$m)=split /\t/,$_,2;
			if(exists $name{$n}){
	...
	foreach my $test (keys %h){
		...
		my $lmtinter = "$lncdir/$test.lm.Trans.Interaction.list";
	        ...
	        while(<IN>){
         	       chomp;
			if(-e $l2mt){
	                	if(/^#/){my ($i1,$d1)=split /\t/,$_,2;print OUT "$i1\tSymbol\t$d1\tdiff_Cis_target_gene\tSymbol\tdiff_Trans_target_gene\tSymbol\tdiff_target_miRNA\n";next;}
			}else{
				if(/^#/){my ($i1,$d1)=split /\t/,$_,2;print OUT "$i1\tSymbol\t$d1\tdiff_Cis_target_gene\tSymbol\tdiff_target_miRNA\n";next;}
			}
		    	my @A = split /\t/,$_;
			my ($n,$m)=split /\t/,$_,2;
                        if(exists $name{$n}){
...
foreach my $circ (keys %h){
	...
	my $cginter = "$circdir/$circ.cm.Interaction.list";
        ...
        while(<IN>){
                chomp;
                if(/^#/){print OUT "$_\tdiff_host_gene\tSymbol\tdiff_target_miRNA\n";next;}
                my @A = split /\t/,$_;
                print OUT "$_\t";
                if(exists $c2g{$A[0]}){
                        my @cg = split /\;/,$c2g{$A[0]};
...
foreach my $mi (keys %h){
	...
	my $scinter = "$sdir/$mi.sc.Interaction.list";
        ...
        while(<IN>){
                chomp;
                if(/^#/){print OUT "$_\tdiff_target_gene\tSymbol\tdiff_target_lncRNA\tSymbol\tdiff_target_circRNA\n";next;}
                my @A = split /\t/,$_;
                print OUT "$_\t";
                if(exists $mi2g{$A[0]}){
...
foreach my $g (keys %h){
	...
	my $gsinter = "$gdir/$g.ms.Interaction.list";
        ...
        while(<IN>){
                chomp;
                if(/^#/){my ($i,$d)=split /\t/,$_,2;print OUT "$i\tSymbol\t$d\tdiff_target_miRNA\tdiff_target_lncRNA\tSymbol\tdiff_target_circRNA\n";next;}
                my @A = split /\t/,$_;
		my ($g,$fpkm)=split /\t/,$_,2;
		if(exists $name{$g}){
	...
	foreach my $g (keys %h){
		...
		my $gltatt = "$gdir/$g.ml.Trans.Attribution.list";
        	...
        	while(<IN>){
        	        chomp;
			if(-e "$l2mt"){
	                	if(/^#/){my ($i,$d)=split /\t/,$_,2;print OUT "$i\tSymbol\t$d\tdiff_target_miRNA\tCis.diff_target_lncRNA\tSymbol\tTrans.diff_target_lncRNA\tSymbol\tdiff_target_circRNA\n";next;}
			}else{
				if(/^#/){my ($i,$d)=split /\t/,$_,2;print OUT "$i\tSymbol\t$d\tdiff_target_miRNA\tCis.diff_target_lncRNA\tSymbol\tdiff_target_circRNA\n";next;}
			}
                my @A = split /\t/,$_;
		my ($m,$p)=split /\t/,$_,2;
		if(exists $name{$m}){

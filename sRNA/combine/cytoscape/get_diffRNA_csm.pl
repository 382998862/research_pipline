...
foreach my $mi (keys %h){
	...
	my $scinter = "$sdir/$mi.sc.Interaction.list";
        ...
        while(<IN>){
                ...
                if(/^#/){
			if(defined $nl){
				print OUT "$_\tdiff_target_gene\tSymbol\tdiff_target_circRNA\n";
			}else{
				print OUT "$_\tdiff_target_gene\tSymbol\tdiff_target_circRNA\n";
			}
			next;
...
foreach my $g (keys %h){
	...
	my $gsinter = "$gdir/$g.ms.Interaction.list";
        ...
        while(<IN>){
			...
			my ($i,$d)=split /\t/,$_,2;
			if(defined $nl){
				print OUT "$i\tSymbol\t$d\tdiff_target_miRNA\tdiff_target_circRNA\n";
			}else{
				print OUT "$i\t$d\tdiff_target_miRNA\tdiff_target_circRNA\n";
			}
			next;
		}
                my @A = split /\t/,$_;

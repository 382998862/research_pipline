...
# create df
xlabel <- c(seq(200,3000,by=200),"3000+")
df <- data.frame(group = data[,1],x=data[,opt$x.col], y=data[,opt$y.col])

#-----------------------------------------------------------------
# plot
#-----------------------------------------------------------------
# mian plot
df$x<-factor(df$x,levels=xlabel)
...
# remove legend
#p<- p +  geom_text(aes(label=y),size=3,hjust=0.5,vjust=-0.5 )
#p <- p + theme_bw()
# percent

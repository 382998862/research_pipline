...
foreach my $sample(sort keys %{$data_cfg{rawdata}})
{
	...
	print CMD "cd $result && ln -snf $detail_cfg{Ref_seq} ./ && ";
	print CMD "$config{bowtie2} -p3 --very-sensitive --mm --score-min=C,-15,0 -x $bowtie2_index/REF --rf -q -U $fq1,$fq2 2> bt2_firstpass.log | $config{samtools} view -hbuS - | $config{samtools} sort - $sample && ";
	print CMD "$config{samtools} view -hf 4 $sample.bam | $config{samtools} view -Sb - > $sample.unmapped.bam && ";
	print CMD "$config{python} $Bin/bin/find_circ/unmapped2anchors.py $sample.unmapped.bam > $sample.qfa && ";
	print CMD "$config{bowtie2} -p3 --reorder --mm --score-min=C,-15,0 -q -x $bowtie2_index/REF --rf -U $sample.qfa 2> bt2_secondpass.log | $config{python} $Bin/bin/find_circ/find_circ.py -G $ref -p 5 -n $sample -s sites.log -R $sample.spliced_reads.fa > $sample.splice_sites.bed && ";
	print CMD "grep CIRCULAR $sample.splice_sites.bed |grep -v chrM |awk '\$5>=2' |grep UNAMBIGUOUS_BP | grep ANCHOR_UNIQUE |$config{python} $Bin/bin/find_circ/maxlength.py 100000 > circ_candidates.bed && sed -i 'N;2i\\$head' circ_candidates.bed\n";
}
close CMD;
`$config{qsub} --resource vf=50G --queue $detail_cfg{Queue_type} --independent $sh_dir/find_circ.sh --reqsub`;
#######################################################################################

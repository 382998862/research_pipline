...
--line.color 	-f 	integer 	the color of line [1-10] [optional, default: 1]
...
colorSet <-c("#4876FF",colorSet)
cat("opt$point.color",opt$point.color,"\nopt$line.color",opt$line.color,"\n")
# line
if( opt$is.line ){
	p <- p + geom_smooth(linetype=opt$linetype, colour=colorSet[opt$line.color], size=opt$line.size,se=F,level=0.99,span=0.1)
}
# point
if( opt$is.point ){

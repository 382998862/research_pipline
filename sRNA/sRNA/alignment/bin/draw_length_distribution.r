...
sample<-unlist(strsplit(args[3],split="[.]"))
...
#p<-qplot(df[,1], df[,2], xlab="Length", ylab="Number", main="Length Graph", size=I(1.5),geom="bar" ,sta="identity",fill=factor(df[,1]))
...
# output plot
file<-paste(args[2],"/",args[3],".length",sep="")
png(filename=paste(file,".png",sep=""), height = 3000, width = 6000, res = 500, units = "px")
print(p)
dev.off()
pdf(file=paste(file,".pdf",sep=""), height = 7, width = 14)
print(p)
dev.off()

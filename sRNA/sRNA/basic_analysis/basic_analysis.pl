...
if($step==3){
	...
	if(defined $fyes){
		runOrDie("perl $Bin/bin/$parse_version -str $idir/miRDeep2/mirdeep_runs/$work_dir/tmp/precursors.str -coord $idir/miRDeep2/mirdeep_runs/$work_dir/tmp/precursors.coords -mrd $idir/miRDeep2/mirdeep_runs/$work_dir/output.mrd -samples $seq -od $idir/miRDeep2/Result -known $idir/Alignment/Ref_Database/$fyes.mature.fa -Tscore $Tscore ");
	}
	else{
		runOrDie("perl $Bin/bin/$parse_version -str $idir/miRDeep2/mirdeep_runs/$work_dir/tmp/precursors.str -coord $idir/miRDeep2/mirdeep_runs/$work_dir/tmp/precursors.coords -mrd $idir/miRDeep2/mirdeep_runs/$work_dir/output.mrd -samples $seq -od $idir/miRDeep2/Result -Tscore $Tscore ");
	}

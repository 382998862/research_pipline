...
#   number of potential star reads that do not map in good consistence with 3' overhang
	...
	if ($query=~/(\w{3})_[^_]+_x\d+/) {
		my $sam=$1;
		$EXP{$sam}{$product}+=$query_freq;
	}
...
sub print_hash_comp{
	...
	if($options{'s'} and defined $hash_comp{"seed_family"}){
        ...
        print "mature read count\t$spacer2s$hash_comp{\"freq_mature\"}\n";
 		foreach my $sam (sort keys %EXP) {
			$EXP{$sam}{mature}||=0;
			$spacer2s = " " x ($spacer2 - length($EXP{$sam}{mature}));
			print "$sam mature read count\t$spacer2s$EXP{$sam}{mature}\n";
		}
       $spacer2s = " " x ($spacer2 - length($hash_comp{"freq_loop"}));
        print "loop read count\t\t$spacer2s$hash_comp{\"freq_loop\"}\n";
 		foreach my $sam (sort keys %EXP) {
			$EXP{$sam}{"loop"}||=0;
			$spacer2s = " " x ($spacer2 - length($EXP{$sam}{"loop"}));
			print "$sam loop read count\t$spacer2s$EXP{$sam}{\"loop\"}\n";
		}
        $spacer2s = " " x ($spacer2 - length($hash_comp{"freq_star"}));
        print "star read count\t\t$spacer2s$hash_comp{\"freq_star\"}\n";
 		foreach my $sam (sort keys %EXP) {
			$EXP{$sam}{"star"}||=0;
			$spacer2s = " " x ($spacer2 - length($EXP{$sam}{"star"}));
			print "$sam star read count\t$spacer2s$EXP{$sam}{\"star\"}\n";
		}

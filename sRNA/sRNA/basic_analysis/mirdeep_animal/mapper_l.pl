...
-p genome       map to genome (must be indexed by bowtie-build-l). The 'genome'
                string must be the prefix of the bowtie index. For instance, if
                the first indexed file is called 'h_sapiens_37_asm.1.ebwtl' then
...
###################################### INPUT #######################################################
###### set env variables
#$ENV{"PATH"} = "/share/nas2/genome/biosoft/miRDeep2/current/:" . $ENV{"PATH"};
...
sub map_reads{
	...
	print MAP "/share/nas1/shitw/software/bowtie-1.1.0/bowtie-align-l  --al $dir/${orig_file_reads}_mapped --un $dir/${orig_file_reads}_not_mapped -p $threads -f -n $mismatches_seed -e 80 -l 18 -a -m $mapping_loc --best --strata $file_genome_latest  $file_reads_latest $dir/mappings.bwt\n\n";
#bowtie -f -n $mismatches_seed -e 80 -l 18 -a -m $mapping_loc --best --strata $file_genome_latest $file_reads_latest $dir/mappings.bwt\n\n";

    my $ret_mapping=`/share/nas1/shitw/software/bowtie-1.1.0/bowtie-align-l --al $dir/${orig_file_reads}_mapped --un $dir/${orig_file_reads}_not_mapped -p $threads -f -n $mismatches_seed -e 80 -l 18 -a -m $mapping_loc --best --strata $file_genome_latest    $file_reads_latest $dir/mappings.bwt`;
...
sub check_options{
    ...
    if($options{p} and $options{p}=~/ebwtl$/){die "please make sure that you are using the -p option correctly.\nThe argument given after -p must be the _prefix_ of the bowtie\nindexed files and should not contain 'ebwtl'. For instance,\nif the first indexed file is called 'h_sapiens_37_asm.1.ebwtl'\nthen the prefix is 'h_sapiens_37_asm'.\n";}

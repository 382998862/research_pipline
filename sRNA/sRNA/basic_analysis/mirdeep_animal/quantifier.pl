...
if($organisms{$species} ){
   $command = "perl $Bin/make_html2.pl -q $outdir/miRBase.mrd -k $name1$extension1 -t $organisms{$species} -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
    ...
    $t=`$command`;
     ...
     $command="perl $Bin/make_html2.pl -q $outdir/miRBase.mrd -k $name1$extension1 -t $species -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
    ...
    $command = "perl $Bin/make_html2.pl -q $outdir/miRBase.mrd -k $name1$extension1 -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
    ...
    while(<IN>){
        ...
        for(my $i = 1; $i <= $hash{$line[2]}{'c'}; $i++){
			if($options{'w'}){## if consider complete precursor as mature seq
				...
				$sample = $1 if($line[0] =~ /^([^_]+)_/); ## get sample id here
...
sub CreateOutputMRD{
	...
	$mature=`perl $Bin/convert_bowtie_output.pl ${name1}_mapped.bwt > ${name1}_mapped.arf`;
    ...
    if($options{'s'}){
        $star=`perl $Bin/convert_bowtie_output.pl ${name3}_mapped.bwt > ${name3}_mapped.arf`;
	...
	$reads=`perl $Bin/convert_bowtie_output.pl ${name2}_mapped.bwt > ${name2}_mapped.arf`;
...
#OLD        print OUT "total read count$spacer",$hash{$k1}{'reads'}{'tot'},"\n"; ### mmm error is here but not seen yet
        ...
        for my $k2 (keys %{$hash{$k1}}){
			...
			#print OUT "read count$spacer$hash{$k1}{$k2}{'score'}\n";   ######## delete the long id
        }

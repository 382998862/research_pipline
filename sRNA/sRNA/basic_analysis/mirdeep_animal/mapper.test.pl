...
#bowtie -f -n $mismatches_seed -e 80 -l 18 -a -m $mapping_loc --best --strata $file_genome_latest $file_reads_latest $dir/mappings.bwt\n\n";
    ...
    if(-f "$ref_dir/$ref_name.1.ebwtl"){
     runOrDie("$BOWTIE_BIN --large-index -p $threads -f -n $mismatches_seed -e 80 -l 18 -a -m $mapping_loc --al $dir/${orig_file_reads}_mapped --un $dir/${orig_file_reads}_not_mapped  $file_genome_latest  $file_reads_latest >$dir/mappings.bwt");
    }
    elsif(-f "$ref_dir/$ref_name.1.ebwt"){
	runOrDie("$BOWTIE_BIN -p $threads -f -n $mismatches_seed -e 80 -l 18 -a -m $mapping_loc --best --strata  --al $dir/${orig_file_reads}_mapped --un $dir/${orig_file_reads}_not_mapped  $file_genome_latest  $file_reads_latest >$dir/mappings.bwt");
    }

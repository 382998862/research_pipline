...
our($opt_q,$opt_r,$opt_h,$opt_v,$opt_t,$opt_a,$opt_m);
# Initialize option defaults
$opt_r = 0.65;  #default minimum free energe ratio
...
my @all_query=<QUERIES>;
close QUERIES;
my $semaphore=new Thread::Semaphore($opt_m);
# Begin work, one query at a time
for(my $i=0;$i<$#all_query;$i++){
    my $header = $all_query[$i];
    my $full_qseq = uc $all_query[++$i];
    ...
    $full_qseq =~ s/T/U/g; ## U's to T's if present
    unless($full_qseq =~ /^[AUGC]+$/) {
    ...
    $semaphore->down();
    my $thr = threads->new(\&parallel_run, \$full_qseq, \%txome, \$header);
    $thr->detach();
}
waitquit(\$opt_m);
...
-m [integer >1]number of threads to lanuch. Default: 1
...
sub check_param{
    ...
    unless(-r $transcripts_file) {
        die "FATAL: $transcripts_file was not readable\n\n$help";
    }
    unless(-r $queries_file) {
        die "FATAL: $queries_file was not readable\n\n$help";
    }
...
sub parallel_run{
    my($r_seq,$r_txome,$r_header)=@_;
    my $q_file = write_q($r_seq,$r_txome,$r_header);  ## The file name is GSTAr_temp[123...].fasta
    # Call the master sub-routine for RNAplex analyses
    my @first_pass = RNAplex_analysis($r_seq,$r_header,$r_txome,\$q_file);
    system "rm -f $q_file";
	...
	if($final_count) {
	    print STDERR "$$r_header : Found $final_count valid alignments\n";
	} else {
	    print STDERR "$$r_header : NO valid alignments found\n";
	}
    }
    if($opt_t) {
...
sub waitquit{
    ...
    unless($opt_q) {
        print STDERR "All query finished.\n\n";
    }
}
...
sub RNAplex_analysis {
    ...
    my $perfect_MFE = get_perfect_MFE($u_qseq,$qfile);
    unless($perfect_MFE) {
...
sub write_q {  ## passed by reference
    my($qseq,$txome,$r_header) = @_;
    my $q_file = "GSTAr_".$$r_header.".tmp";
...
sub get_perfect_MFE {
    ...
    my $outfile=$$qfile.".perfect";
    (open(TEMP, ">$outfile")) || die "ABORT: Failed to create a temp file in sub-routine get_perfect_MFE\n";
    print TEMP ">perfect\n$perfect\n>Query\n$seq\n";
    close TEMP;
    
    # Call RNAplex under default parameters
    (open(PLEX, "RNAplex < $outfile |")) || die "ABORT: RNAplex call failed in sub-routine get_perfect_MFE\n";

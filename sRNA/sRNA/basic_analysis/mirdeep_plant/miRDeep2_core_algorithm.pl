...
-n Normalization with pri-miRNA length, suggested by Thakur et al. 2011.
...
#options
my %options=();
getopts("hs:tuv:xy:zl:n:",\%options);
...
#    potential precursor in turn
	...
	if($line=~/^(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)/){
	    ...
	    my $edits=$12;
	    my $edit_string=$13;
        #next if exists $hash_randfold{$db}
        if($options{'y'}){
            next unless exists $hash_randfold{$db};
        }
...
sub parse_file_struct{
        ...
        if (/^>(\S+)\s*(.*)/)
	{
	    ...
	    $struct      = "";
	    $mfe         = "";
        #next if exists $hash_randfold{$id};
        if($options{'y'}){
            next unless exists $hash_randfold{$id};
        }
        #print STDERR $id;
	    while (<FILE_STRUCT>){
                chomp;
...
sub print_hash_comp{
	...
	if($options{'s'} and defined $hash_comp{"seed_family"}){
        ...
        print "mature_arm$spacer",$hash_comp{"mature_arm"};
        print "\nmature_beg$spacer",$hash_comp{"mature_beg"};
        print "\nmature_end$spacer",$hash_comp{"mature_end"};
        print "\nmature_query$spacer",$hash_comp{"mature_query"};
        print "\nmature_seq$spacer",lc $hash_comp{"mature_seq"};
        print "\nmature_str$spacer",$hash_comp{"mature_struct"};
        print "\npre_seq$spacer",lc $hash_comp{"pre_seq"};
        print "\npre_str$spacer",$hash_comp{"pre_struct"};
        print "\nstar_arm$spacer",$hash_comp{"star_arm"};
        print "\nstar_beg$spacer",$hash_comp{"star_beg_exp"};
        print "\nstar_end$spacer",$hash_comp{"star_end_exp"};
        print "\nstar_seq$spacer",lc $hash_comp{"star_seq"};
        print "\nstar_str$spacer",$hash_comp{"star_struct"};
        print "\nloop_seq$spacer",lc $hash_comp{"loop_seq"},"\n";
...
sub score_length_monocot{
    my($mfe,$pri_lng)=@_;
    my $mfe_adj=$mfe/$pri_lng;
    my $param_a=1.339e-12;
    my $param_b= 2.77826e-13;
    my $param_c= 45.8426; 
    my $log_odds=0;
    $log_odds=$param_a/($param_b+exp($mfe_adj*$param_c));
}
sub score_length_dicot{
    my($mfe,$pri_lng)=@_;
    my $mfe_adj=$mfe/$pri_lng;
    my $param_a=4.46e-4;
    my $param_b= 9.125e-5;
...
sub score_mfe{
    ...
    if($options{n}){
        if($options{n} eq 'm'){
            $log_odds=score_length_monocot($mfe,$pri_lng);
        }
        elsif($options{n} eq 'd'){
            $log_odds=score_length_dicot($mfe,$pri_lng);
        }
        else{die "The value should be one of 'd' or 'm'!\n";}
    }
    else{
        $log_odds=score_max($mfe);
    }
    return $log_odds;
}

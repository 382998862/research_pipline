...
#-----------------------------------------------------------------
# reading data
#opt=data.frame(infile="S01.error.stat",outfile="error",x.col=1,y.col=2,axis.size=8)
#opt$infile="S01.error.stat"
#opt$skip = 0
...
p <- p + theme_bw()+theme(title = element_text(face="bold", size=opt$lab.size), 
...
p<-p+theme(axis.text.x = element_text(angle = 315, size = 8,hjust = 0.5, vjust = 0.5))

...
if($step==1) {
	...
	$cmd = "perl $Bin/$diff_perl -in $idir -cfg $cfg -od $odir ";
...
if($step==2) {
	...
	$cmd = "perl $Bin/$venn_perl -in $idir -cfg $cfg -od $odir ";
	print SH "$cmd\n";
        close(SH);
...
#        $cmd = "perl $Bin/get_ceRNA_cytoscape.pl -in $idir -ce $ceRNA -od $odir -cfg $cfg";
#        print SH "$cmd\n";
#        close(SH);
...
----------------------------------------------------------------------------------------------------------------------------
   ...
   Contact: niepy <niepy\@biomarker.com.cn> 
     ...
     Usage:
            ...
            -in	<DIR>	analysis/Combine/Input
            -od		<DIR>	Combine/cytoscape
	    -list	<file>	gene2name.list
   ...
   Example:
            perl $Script -cfg combine.cfg -in Input -od Combine/cytoscape -id_name gene2name -step 1

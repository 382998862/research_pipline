...
if(-f "$db.1.ebwt"){
if ($sam) {
	system "bowtie $db -v $mis -p 5 -S  -f $fa $out ";
}
else{
	system "bowtie $db -v $mis -p 5  -f $fa $out";
}
}
elsif(-f "$db.1.ebwtl"){
if ($sam) {
	system "bowtie $db -v $mis -p 5 -S  -f $fa $out --large-index";
}
else{
	system "bowtie $db -v $mis -p 5  -f $fa $out --large-index";
}
}
######################################### subs
sub ABSOLUTE_DIR

...
def _Round0(bowtie, refpath, libname, libpath, format, filter, filterpath, root0):
	...
	cmd2 = "%s --norc -n 0 -v 0 -a -l 6 -t -%s  --sam --un %sunmapped/unr0-%s.%s --al %smapped/r0-%s.%s %s  %sunmapped/unr0tmp-%s.%s  >%sSAM/r0tmp-%s.sam " %(bowtie,  inp,root0, libname, format, root0, libname, format,filterpath,root0, libname, format, root0, libname)
	print "cmd2:\n"
	print cmd2

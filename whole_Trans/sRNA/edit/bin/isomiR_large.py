        ...
        if mod != "no":
            ...
            for i in range(0, len(Seq)):
                if i == position:
                    ...
                    newSeq += Seq[i]
        return newSeq
    def __ModifySeq(Seq, modif, mod):
        newSeq = ""
        if modif != "no":
            position = int(modif.split(",")[1])-1
            for i in range(0, len(Seq)):
                if mod == "M3":
        ...
        for r in refs:
            ...
            if r not in dic:
                ...
                if dic[r][SeqID]["Modification"] != "no":
                    if rn == "r2":
                        dic[r][SeqID]["ModSeq"] = __ModifySeq(seq, dic[r][SeqID]["Modification"], dic[r][SeqID]["Mod"])
                    else:
...
def _freqSAM(input1, n, R, output, nadd):
    ...
    cab = "OriginalSeq"+"\t"+"Round"+"\t"+"Length"+"\t"+"Variation"+ cab + "\n"
    out.write(cab)
...
def _Round0(bowtie, refpath, libname, libpath, format, filter, filterpath, root0):
    ...
    if format == "fa":
        ...
        inp = "q"
    if filter == "yes":
        cmd1 = "%s --norc -n 0 -v 0 -a -l 6 -t %s -%s %s --sam %sSAM/r0-%s.sam --un %sunmapped/unr0tmp-%s.%s --al %smapped/r0-%s.%s" %(bowtie, refpath, inp, libpath, root0, libname, root0, libname, format, root0, libname, format)
	print "cmd1:\n"
	print cmd1
	...
	cmd2 = "%s --norc --large-index -n 0 -v 0 -a -l 6 -t -%s  --sam --un %sunmapped/unr0-%s.%s --al %smapped/r0-%s.%s %s  %sunmapped/unr0tmp-%s.%s  >%sSAM/r0tmp-%s.sam " %(bowtie,  inp,root0, libname, format, root0, libname, format,filterpath,root0, libname, format, root0, libname)
	print "cmd2:\n"
	print cmd2
...
def _Complete_analysis(configfile):
    bowtie, libs, mainref, filterref, mirnaref, add3, add5, Size_cutoff, cutoffs = _ReadConfig(configfile)
...
if __name__=="__main__":
    ...
    print "\n Start Running isomiR Version 0.53\n"
    configfile = sys.argv[1]
    print configfile

    if "-f" in sys.argv or "--force" in sys.argv:
        _cleanALLDIRS()
        _Complete_analysis(configfile)
    else:
        _Complete_analysis(configfile)

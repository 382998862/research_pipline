...
################################
	open IN,"<$file_reads";                  ############Total_reads_collapsed.fa
	my $line=<IN>;
	chomp $line;
	if($line !~ /^>\S+/){
		printErr();
		die "The first line of file $file_reads does not start with '>identifier' Reads file $file_reads is not a valid fasta file\n\n";
	}
	if($line =~ /\s/){
		printErr();
	...
	if(<IN> !~ /^[ACGTUNacgtun]*$/){
		printErr();
		die "File $file_reads contains not allowed characters in sequences Allowed characters are ACGTUN Reads file $file_reads is not a fasta file\n\n";
	}
	close IN;

	open IN,"<$file_genome";		##########reference genome
	$line=<IN>;
	chomp $line;
	if($line !~ />\S+/){
		printErr();
		die "The first line of file $file_genome does not start with '>identifier' Genome file $file_genome is not a fasta file\n\n";
	}
	if($line =~ /\s/){
		printErr();
	...
	if(<IN> !~ /^[ACGTUNacgtun]*$/){
		printErr();
		die "File $file_genome contains not allowed characters in sequences Allowed characters are ACGTUN Genome file $file_genome is not a fasta file\n\n";
	}
	close IN;

	open IN,"<$file_reads_vs_genome";		########Total_reads_collapsed_vs_genome.arf
	...
	if($file_mature_ref_this_species !~ /none/){		######*.mature.fa
		open IN,"<$file_mature_ref_this_species";
		my $line=<IN>;
		chomp $line;
		if($line !~ />\S+/){
			printErr();
			die "The first line of file  $file_mature_ref_this_species does not start with '>identifier' miRNA reference this species file $file_mature_ref_this_species is not a fasta file\n\n";
		}
		if($line =~ /\s/){
			printErr();
		...
		if($line !~ /^[ACGTUNacgtun]*$/){
			printErr();
			die "File $file_mature_ref_other_species contains not allowed characters in sequences Allowed characters are ACGTUN miRNA reference this species file $file_mature_ref_this_species is not a fasta file\n\n";
		}
		close IN;
	}
	...
	if($file_mature_ref_other_species !~ /none/){
		...
		if($line !~ />\S+/){
		printErr();
		die "The first line of file  $file_mature_ref_other_species does not start with '>identifier' miRNA reference other species file $file_mature_ref_other_species is not a fasta file\n\n";
		}
		if($line =~ /\s/){
			printErr();
		...
		if(<IN> !~ /^[ACGTUNacgtun]*$/){
			printErr();
			die "File $file_mature_ref_other_species contains not allowed characters in sequences Allowed characters are ACGTUN miRNA reference other species file $file_mature_ref_other_species is not a fasta file\n\n";
		}
		close IN;
	}
	...
	if($file_precursors !~ /none/){
		...
		if($line !~ />\S+/){
			printErr();
			die "The first line of file $file_precursors does not start with '>identifier' precursor file $file_precursors is not a fasta file\n\n";
		}
		if($line =~ /\s/){
			printErr();
		...
		if($line !~ /^[ACGTUNacgtun]*$/){
			printErr();
			die "File $file_precursors contains not allowed characters in sequences Allowed characters are ACGTUN precursor file $file_precursors is not a fasta file\n\n";
		}
		if(length($line) < $minpreslen){
			printErr();
			die "The precursor file $file_precursors does not contain sequences of at least $minpreslen nt Please make sure that you provided the correct file and the correct parameter ordering when calling miRDeep2.pl; If you have precursors with less than $minpreslen please use option -p <int> to specify this length\n";
...
##################################################
	...
	print STDERR "perl $Bin/sanity_check_reads_ready_file.pl $file_reads\n\n";
	start();
	my $ret_test_file_reads=`perl $Bin/sanity_check_reads_ready_file.pl $file_reads 2>&1`;
	...
	start();
	print STDERR "perl $Bin/sanity_check_genome.pl $file_genome\n\n";
	my $ret_test_file_genome=`perl $Bin/sanity_check_genome.pl $file_genome 2>&1`;
	...
	if($file_precursors !~ /none/){
	    ...
	    if($options{'d'}){$dopt="-d";}
            ...
            my $quant = "perl $Bin/quantifier.pl -p $file_precursors -m $file_mature_ref_this_species  -r $file_reads $file_star $species -y $time -k $dopt $Popt";
            print STDERR $quant,"\n";
            `$quant`; # not quantify
...
sub parse_mappings{
	...
	print STDERR "perl $Bin/parse_mappings.pl $file_reads_vs_genome -a 0 -b 18 -c 30 -i 5 > $dir_tmp/${parsed_arf}_parsed.arf\n\n";
	start();


	#my $ret_parse_mappings=`parse_mappings.pl $file_reads_vs_genome -a 0 -b 18 -c 25 -i 5 > $dir_tmp/${parsed_arf}_parsed.arf`;
	my $ret_parse_mappings=`perl $Bin/parse_mappings.pl $file_reads_vs_genome -a 0 -b 18 -c 30 -i 5 > $dir_tmp/${parsed_arf}_parsed.arf`;
	end();
...
sub excise_precursors{
    ...
    if($options{a}){
        ...
        print STDERR "perl $Bin/excise_precursors_iterative_final.pl $file_genome $dir_tmp/${parsed_arf}_parsed.arf $dir_tmp/precursors.fa $dir_tmp/precursors.coords $max_pres\n";
        $ret_excise_precursors=`perl $Bin/excise_precursors_iterative_final.pl $file_genome $dir_tmp/${parsed_arf}_parsed.arf $dir_tmp/precursors.fa $dir_tmp/precursors.coords $max_pres`;
...
#	}
	my %FA1;
	&LOAD_SEQ("$dir_tmp/precursors.fa",\%FA1);
	mkdir "$dir_tmp/precursors_cut" unless -d "$dir_tmp/precursors_cut";
	mkdir "$dir_tmp/work_sh" unless -d "$dir_tmp/work_sh";
	&CUTFA(\%FA1,"$dir_tmp/precursors_cut",10000,'precursors');
	my $now_work_dir=`pwd`;chomp $now_work_dir;
	open (SH,">$dir_tmp/work_sh/RNAfold.sh") or die $!;
	foreach my $file (glob "$now_work_dir/$dir_tmp/precursors_cut/precursors.*.fa") {
		print SH "RNAfold < $file --noPS > $file.str \n";
	}
	close SH;
	qsubOrDie ("$now_work_dir/$dir_tmp/work_sh/RNAfold.sh",$CFG{queue},$CFG{cpu},$CFG{vf});
	`cat $now_work_dir/$dir_tmp/precursors_cut/precursors.*.fa.str > $now_work_dir/$dir_tmp/precursors.str `;
...
#    my $ret_randfold=`randfold -s $dir_tmp/precursors_for_randfold.fa 99 > $dir_tmp/precursors_for_randfold.rand`;
	my %FA2;
	&LOAD_SEQ("$dir_tmp/precursors_for_randfold.fa",\%FA2);
	mkdir "$dir_tmp/randfold_cut" unless -d "$dir_tmp/randfold_cut";
	mkdir "$dir_tmp/work_sh" unless -d "$dir_tmp/work_sh";
	&CUTFA(\%FA2,"$dir_tmp/randfold_cut",2000,'randfold');
	my $now_work_dir=`pwd`;chomp $now_work_dir;
	open (SH,">$dir_tmp/work_sh/randfold.sh") or die $!;
	foreach my $file (glob "$now_work_dir/$dir_tmp/randfold_cut/randfold.*.fa") {
		print SH "$CFG{randfold} -s $file 99 > $file.rand \n";
	}
	close SH;
	qsubOrDie("$now_work_dir/$dir_tmp/work_sh/randfold.sh",$CFG{queue},$CFG{cpu},$CFG{vf});
	`cat $now_work_dir/$dir_tmp/randfold_cut/randfold.*.fa.rand > $dir_tmp/precursors_for_randfold.rand `;
    end();
    return;
}
...
sub miRDeep_core_algorithm{
    ...
    if($file_mature_ref_other_species !~ /none/i){
        ...
        $line="perl $Bin/miRDeep2_core_algorithm.pl $dir_tmp/signature.arf $dir_tmp/precursors.str -s $dir_tmp/$file_mature_ref_other_species -v -50 -l $longest_id";

    }else{

        $line="perl $Bin/miRDeep2_core_algorithm.pl $dir_tmp/signature.arf $dir_tmp/precursors.str -v -50 -l $longest_id";
    }
...
sub perform_controls{
    ...
    print STDERR "perl $Bin/perform_controls.pl $dir_tmp/command_line $dir_tmp/precursors.str 100 -a > $dir_tmp/output_permuted.mrd 2>>error_${time}.log\n\n";
    my $ret_perform_controls=`perl $Bin/perform_controls.pl $dir_tmp/command_line $dir_tmp/precursors.str 100 -a > $dir_tmp/output_permuted.mrd 2>>error_${time}.log`;
    end();
    return;
}
...
sub output_results{
    ...
    if($file_mature_ref_this_species !~ /none/i){
        ...
        if($options{'q'}){
            $line="perl $Bin/make_html.pl -f $dir/output.mrd -k $dir_tmp/$file_mature_ref_this_species -p $dir_tmp/precursors.coords -s $dir/survey.csv -c -e -q $options{'q'} -x $xopt -r $Bin/Rfam_for_miRDeep.fa -v $sc -y $time $sort_by_sample $OE";
        }else{
            $line="perl $Bin/make_html.pl -f $dir/output.mrd -k $dir_tmp/$file_mature_ref_this_species -p $dir_tmp/precursors.coords -s $dir/survey.csv -c -e -r $Bin/Rfam_for_miRDeep.fa -v $sc -y $time  $sort_by_sample $OE";
        }
    }else{
        if($options{'q'}){
            $line="perl $Bin/make_html.pl -f $dir/output.mrd -p $dir_tmp/precursors.coords -s $dir/survey.csv -c -e -q $options{'q'}  -x $xopt -r $Bin/Rfam_for_miRDeep.fa -v $sc -y $time $sort_by_sample $OE";
        }else{
            $line="perl $Bin/make_html.pl -f $dir/output.mrd -p $dir_tmp/precursors.coords -v $sc -s $dir/survey.csv -c -e -r $Bin/Rfam_for_miRDeep.fa -y $time $sort_by_sample $OE";
        }
    }
...
#	die "Error: \tPerl PDF::API2 package not found\nCheck if the perl PDF::API2 package is correctly installed and all Pathes were set correctly.\n$stdm" if($ret);
	...
	if(not -f "$Bin/Rfam_for_miRDeep.fa"){

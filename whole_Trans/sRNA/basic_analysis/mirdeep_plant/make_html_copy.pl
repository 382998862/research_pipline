...
my %mature_pos_hash;
if($options{'a'} and -f "mirdeep_runs/tmp/signature.arf"){
    get_mature_pos();
}
...
while(<IN>){
	...
	elsif(/^(\S+)(_\d+_x\d+)\s+(\S+)\s+(\S+)$/ and $reads){  ## do not read in known miRNAs
	    $counter++;
...
sub CreatePDF{
	...
	#$pdf->preferences(
	#	-twocolumnright=>1,
	#	-firstpage=>[$page,-xyz=>[1,1,1]],
	#);
	
	###
=c	
	my $left_column_text=$page->$pdf;
...
sub ReadInParameters(){
	if(-f "mirdeep_runs/run_${time}_parameters"){
		print HTML "<h2>Parameters used</h2>\n";
		print HTML " <table border=\"0\">\n";
		print HTML "<tr><td>miRDeep2 version</td><td>$options{'V'}</td></tr><br>\n";
		open PAR,"<mirdeep_runs/run_${time}_parameters" or print STDERR "File with parameters could not be openend\n";
		while(<PAR>){
			if(/args\s+(\S.+)/){
		...
		print STDERR "File mirdeep_runs/run_${time}_parameters not found\n";
	}	
}
...
# 		close CSV;
		...
		open IN,"expression_analyses/expression_$options{'y'}.html" or die "No file found called expression_analyses/expression_$options{'y'}.html\n";
		while(<IN>){
...
# 		 }
    ...
    open IN,"expression_analyses/miRNA_not_expressed.csv" or die "expression_analyses/miRNA_not_expressed.csv not found\nplease run quantifier module again and check for errors\n\n";
...
sub check_Rfam{
    ...
    my $err;
    if($options{'q'}){
        open TMP,">expression_analyses/identified_precursors.fa" or die "Error: could not create file expression_analyses/identified_precursors.fa\n";
    }else{
        open TMP,">mirdeep_runs/identified_precursors.fa" or die "Error: could not create file mirdeep_runs/identified_precursors.fa\n";
    }
    my ($seqm,$seql,$seqs);
    for(keys %hash){
        chomp;
...
The Rfam analysis will be skipped as long as this is not possible\n\n";
    ...
    if($options{'q'}){
        $err = `bowtie -f -v 1 -a --best --strata --norc ${scripts}indexes/Rfam_index expression_analyses/identified_precursors.fa expression_analyses/rfam_vs_precursor.bwt`;
        open IN,"<expression_analyses/rfam_vs_precursor.bwt" or die "Error: file expression_analyses/rfam_vs_precursor.bwt not found\n";
    }else{
        $err = `bowtie -f -v 1 -a --best --strata --norc ${scripts}indexes/Rfam_index mirdeep_runs/identified_precursors.fa mirdeep_runs/rfam_vs_precursor.bwt`;
        open IN,"<mirdeep_runs/rfam_vs_precursor.bwt" or die "Error: file mirdeep_runs/rfam_vs_precursor.bwt not found\n";
    }
...
sub get_mature_pos{
    ...
    open IN,"mirdeep_runs/tmp/signature.arf" or die "no signature file given for parsing\n";

...
sub parse_genome_lengths{
    ...
    $$hash_db_lng{$id} = length $sequence;
    close FASTA;
    open OUT, ">chr_length" or die "can not create file chr_length\n";
    foreach(keys %{$hash_db_lng}){
        print OUT join("\t",$_,$hash_db_lng->{$_})."\n";
    }
    close OUT;
}
    ...
    foreach my $strand(@strands){        
        ...
        foreach my $db_beg(@db_begs){
            ...
            foreach my $db_end(@db_ends){                
                ...
                my $freq_max_ds=find_freq_max_downstream($$db,$strand,$db_beg,$short_length);
                ...
                for (my $z=1;$z<$upper_bound;$z++){
                    ...
                    my $excise_beg=$db_end-$short_length;
                    ...
                    $excise_end=$db_beg+$short_length;
...
sub find_freq_max_downstream{
    ...
    my ($db,$strand,$db_beg,$short_length)=@_;

    my $freq_max=0;
    
    for(my $pos_beg=$db_beg+1; $pos_beg<=$db_beg+$short_length; $pos_beg++){

...
our($mode,$opt_h,$opt_v,$opt_q,$opt_d,$opt_e,$opt_p,$opt_o,$opt_g,$opt_u,$opt_n,$opt_a,$opt_r,$opt_t,$opt_c,$opt_m);

# Initialize opt_p and opt_c and opt_m defaults
$opt_p = 1;
$opt_c = 4;
$opt_m = 1; #default number of threads

# get options
getopts('o:d:e:g:u:n:p:r:c:m:hvqlat');
$mode = determine_mode();
check_param();
...
my $GSTAr_file;
if(($mode == 1) or ($mode == 2)) {
    my $GSTAr_txome_file=make_GSTAr_txome();
    $GSTAr_file = make_GSTAr_file($GSTAr_txome_file);
    #system "rm -f $GSTAr_txome_file";
} 
else {
    $GSTAr_file = $opt_g;
}
...
while (<G>) {
    ...
    if($_ =~ /^Query\tTranscript\tTStart/) {
        if($opt_t) {
            my @tmp=split /\t/,$_;
            my $head_line=join("\t",(@tmp[0..4],"TrueTSlice","SliceType",@tmp[5..$#tmp]));
            print "SiteID\t$head_line\tDegradomeCategory\tDegradomePval\tTplot_file_path\n";
        }
        next; ## column names
    }
    @gfields = split ("\t", $_);
    ...
    ($cat,$slice_type) = check_cat(\%deg_data,\$gfields[1],\$gfields[4]);
    ...
    $true_slice=$gfields[4]+10-$slice_type;
    
    $chance = $deg_header{$cat} / $deg_header{'tx_ef_size'};
    $p_val = 1 - (pbinom(0,$rank,$chance));
    
    $slice_site = "$gfields[1]" . ":" . $gfields[0] . ":" . $true_slice;
    if(($p_val <= $opt_p) and ($cat_digit <= $opt_c)) {
        $ext_entry = join("\t",(@gfields[0..4],$true_slice,$slice_type,@gfields[5..$#gfields],$cat_digit,$p_val));
...
-m [integer >=1] : number of threads to launch. Default is 1.
...
sub make_deg_density {
    ...
    if($opt_q) {
        system "bowtie -f -v 1 --best -k 1 -p $opt_m --norc -S $opt_n $opt_e 2> /dev/null \| sed -e 's/SO:unsorted/SO:coordinate/' 2> /dev/null \| samtools view -S -b -u - 2> /dev/null \| samtools sort - $bam_name 2> /dev/null";
    } 
    else {
        system "bowtie -f -v 1 --best -k 1 -p $opt_m --norc -S $opt_n $opt_e \| sed -e 's/SO:unsorted/SO:coordinate/' \| samtools view -S -b -u - 2> /dev/null \| samtools sort - $bam_name 2> /dev/null";
    }
    ...
    my $dd_file_tmp = basename($opt_e) . "_dd.txt" . ".tmp";
...
sub make_GSTAr_file {
    ...
    my $file = basename($opt_u) . "_GSTAr.txt";
    $command .= " > $file";
...
sub check_cat {
    ...
    foreach my $line (@lines) {
        ...
        elsif($fields[0] == $$slice) {
            $cat_num = $fields[2];
            $slice_type=10;
            last;
        }
        elsif($fields[0] == $$slice+1){
            if($reads_num<$fields[1]){
                $cat_num = $fields[2];
                $slice_type=9;
            }
            last;
...
sub pretty_report_hit {
    ...
    print "\n5\' ";
    my @tseq = get_al_array($fields[14],0);
    print (@tseq," 3\'");
    print " Transcript: $fields[1]",":","$fields[2]","-","$fields[3]"," Slice Site:$fields[4]"," Possible True Slice Site:$fields[5]\n";
    my @tbrax = get_al_array($fields[13],0);
    my @qbrax = reverse(get_al_array($fields[13],1));
    my @qseq = reverse(get_al_array($fields[14],1));
    my $i = -1;
    print "   ";
    foreach my $tb (@tbrax) {
        ++$i;
    ...
    print "SiteID: $key\n";
    print "MFE of perfect match: $fields[7]\n";
    print "MFE of this site: $fields[8]\n";
    print "MFEratio: $fields[9]\n";
    print "Allen et al. score: $fields[10]\n";
    my @paired = split ("\;", $fields[11]);
...
sub make_t_plot {
    ...
    my $args2 = "$dir_name" . "\/" . "$gfields[0]" . "_" . "$gfields[1]" . "_" . "$gfields[5]" . "_TPlot.pdf";
    my $args3 = "T\=$gfields[1]" . "_" . "Q\=$gfields[0]" . "_" . "S\=$gfields[5]";
...
sub make_GSTAr_txome{
    open IN, $opt_n;
    my $outfile="tx_for_GSTAr.tmp.fasta";
    (open OUT, ">$outfile") || die "cannot create $outfile";
    unless($opt_q){
        print STDERR "Create tmp transcriptome file: $outfile.\n";
    }
    my $index=0;
    while(<IN>){
        if(/^>(\S+)/){
            my $tid=$1;
            $index=0;
            if(exists $deg_tx_lens{$tid}){
                $index=1;
            }
        }
        print OUT $_ if $index==1;
    }
    close IN;
    close OUT;
    return $outfile;
...
7: TrueTSLice: One-based position of the alignment opposite position of the query that could be found in degradome density file

8: SLiceType: The position of the query opposite TrueTSlice position. Could be 9, 10, 11

9: MFEperfect: Minimum free energy of a perfectly matched site (approximate)

10: MFEsite: Minimum free energy of the alignment in question

11: MFEratio: MFEsite / MFEperfect

12: AllenScore: Penalty score calculated per Allen et al. (2005) Cell, 121:207-221 [PMID: 15851028].

13: Paired: String representing paired positions in the query and transcript. The format is Query5'-Query3',Transcript3'-Transcript5'.  Positions are one-based. Discrete blocks of pairing are separated by ;

14: Unpaired: String representing unpaired positions in the query and transcript. The format is Query5'-Query3',Transcript3'-Transcript5'[code].  Possible codes are "UP5" (Unpaired region at 5' end of query), "UP3" (Unpaired region at 3' end of query), "SIL" (symmetric internal loop), "AILt" (asymmetric internal loop with more unpaired nts on the transcript side), "AILq" (asymmetric internal loop with more unpaired nts on the query side), "BULt" (Bulged on the transcript side), or "BULq" (bulged on the query side). Positions are one-based. Discrete blocks of pairing are separated by ;

15: Structure: Aligned secondary structure. The region before the "&" represents the transcript, 5'-3', while the region after the "&" represents the query, 5'-3'.  "(" represents a transcript base that is paired, ")" represents a query based that is paired, "." represents an unpaired base, and "-" represents a gap inserted to facilitate alignment.

16: Sequence: Aligned sequence. The region before the "&" represents the transcript, 5'-3', while the region after the "&" represents the query, 5'-3'. 

17: DegradomeCategory:
...
18: DegradomePval: p-value for this degradome hit.

19: Tplot_file_path: File path for the T-plot of this hit.

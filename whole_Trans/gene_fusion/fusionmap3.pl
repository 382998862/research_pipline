...
for(my $i=0;$i<@Bam_file;$i++){
  ...
  if ($type eq "Human.B38") {
	...
	print SH "&&cat $odir/fusionmap_result/$ID[$i]/$ID[$i]\.PairedEndFusionReport.txt | awk '\$4>10 {print \$0}'|grep -v -E 'InBlackList|InFamilyList|InParalogueList|SameEnsemblGene' >$odir/fusionmap_result/$ID[$i]/$ID[$i]\.FusionReport_filter.txt&&sed  's/HISAT_aln.sorted.bam\.//g' -i $odir/fusionmap_result/$ID[$i]/$ID[$i]\.FusionReport_filter.txt\n";
...
for (my $i=0;$i<@ID ;$i++) {
 ...
 print SH "&& cp $odir/result/$ID[$i]/circos\.svg $odir/result/png/$ID[$i]\_circos\.svg ";
...
&run_or_die("cat $odir/result/report/*_FusionReport.xls | less | grep -v FusionID | sort | uniq > $odir/pfam/fusion.gtf");
...
if ($gene_num<=30000){
	...
	print "the number of fusiongenes > 30000 and the pfam annotation is skipped";
}

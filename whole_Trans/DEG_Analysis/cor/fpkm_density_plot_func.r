...
sam_name <- colnames(fpkm_data)[ !(colnames(fpkm_data)%in%c("geneLength")) ]
print(sam_name)
gene_name <- rownames(fpkm_data)
fpkm <- as.matrix(fpkm_data[ ,sam_name ])
colnames(fpkm)<-sam_name
rownames(fpkm)<-gene_name
...
require(edgeR)
fpkm<-as.matrix(fpkm[rownames= rownames(count_data),])
colnames(fpkm)<-sam_name
log10_fpkm <- fpkm

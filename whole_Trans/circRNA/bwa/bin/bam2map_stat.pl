...
&run_or_die("$config{samtools} view $bam > $bam.txt");
...
while (<IN>) {
    ...
    if (64 & $flag) {
        ...
        $left_uniq_mapped_reads{$qid} = 1 if ($mapQ !=0);
    } else {
        $right_mapped_reads{$qid} = 1;
        $right_uniq_mapped_reads{$qid} = 1 if ($mapQ !=0);
    }
...
my %map_strand;
&run_or_die("$config{bedtools} bamtobed -i $bam > $bam.bed");
...
&run_or_die("$Rscript $Bin/pointOrLine.r --infile $od/$Index.insertSize.r.list --outfile $od/$Index.insertSize.png --x.col 1 --y.col 2 --x.lab \"Insert Size (bp)\" --y.lab \"Number of Reads\" --is.line --line.color 1 --no.grid");
system "rm $bam.txt";
system "rm $bam.bed";
#########################subs
sub MKDIR

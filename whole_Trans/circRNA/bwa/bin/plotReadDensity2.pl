...
if (defined $medical) {
	my $cmd = "$Rscript $Bin/plot_coverage_distribution_v2.r  --input1 $plus.stat --input2 $minus.stat --output $dOut/$fKey.readDensity.png -C $Bin/medical.chr.list -w $window -x \"$x_title\" -y \"$y_title\" -t \"$title\"";
	print $cmd,"\n";
	process_cmd($cmd);
}else {
	$nchro=($chrnum>20)?10:$chrnum	unless(defined $nchro);
	my $cmd = "$Rscript $Bin/plot_coverage_distribution_v2.r  --input1 $plus.stat --input2 $minus.stat --output $dOut/$fKey.readDensity.png  -w $window -n $nchro -x \"$x_title\" -y \"$y_title\" -t \"$title\"";
	print $cmd,"\n";
	process_cmd($cmd);
}

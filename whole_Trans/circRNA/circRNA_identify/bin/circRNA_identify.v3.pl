...
my $cmd;
my $CIRI_dir = "$Bin/bin/CIRI_v2.0.5.pl";
my @bwa_files = glob("$bwa_dir/*/*.sam");
open SH,">$sh_dir/CIRI.sh";
foreach my $file(@bwa_files){
	my $outcirc = basename($file);
	$outcirc =~s/sam/circ/;
	print SH "perl $CIRI_dir -I $file -O $odir/$outcirc -F $ref -A $gff -T $threads";
	print SH " -E $re" if(defined $re);
	print SH "\n";
}
close SH;
&run_or_die("$config{qsub} --resource vf=50G --queue \"medical.q\" --independent --reqsub $sh_dir/CIRI.sh");
#######################################################################################

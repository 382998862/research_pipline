...
if ($step == 2) {
    ...
    if (defined $eggNOG ||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_eggNOG_Anno.pl  -id $mid_dir -Database $CFG{eggNOG} -od $odir -cpu $Blast_cpu -Blast_e $Blast_e  -queue  $queue\n";
		close SH;
		push @work_sh,$SH;
    }
    ...
    if (defined $Pfam||defined $all) {
		...
		open (SH,">$SH") or die $!;
		#if ($CFG{'Lib_type'} eq "fr-secondstrand" or $CFG{'Lib_type'} eq "fr-firststrand"){
			print SH "perl $Bin/bin/Gene_Pfam_Anno.pl --idir $mid_dir --pfam $CFG{Pfam} --odir $odir --cpu $hmmscan_cpu -queue  $queue -ss \n";
		#} elsif (!exists $CFG{'Lib_type'} || $CFG{'Lib_type'} eq "fr-unstranded")
		#{

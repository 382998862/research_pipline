...
print_usage <- function(spec=NULL){
  ...
  cat("
      ...
      --bg	-b 	character 		set the backgroud [optional, default: transparent]
...
if ( is.null(opt$bg ) )  { opt$bg= "transparent"}
...
#plot pie charts
pieplot<-function(x,legend=NULL,labels=NULL,title="",outfile="pie.plot",height=3000,width=4000,legend.pos="right",col="Set1",is.axis=F,lab.size=5,legend.size=10,no.grid=T,legend.ncol=1,bg=NA){  
  library(ggplot2)
  ...
  if(length(col)==1){
   ...
   col<-c("#0000ff","#9400D3","#339900","#ffcc00","#00ccff")
  ...
  if((!is.null(labels))&&length(labels)==length(x)){
    ...
    for(i in 1:length(x)){
      ...
      if(x[i]<=0){
        ...
        j=j+nchar(labels[i])/2*lab.size*0.4;
      }else{ 
        xat=c(xat,0.9)
        yat=c(yat,(df$y/2 + c(0, cumsum(df$y)[-length(df$y)]))[i])
        j=0
  ...
  p <- ggplot(df, aes(x="", y=y, fill=x,)) + geom_bar(width = 1, stat="identity",alpha=0.6)
...
pie.data<- read.delim(opt$infile,sep="\t",header = T,row.names=1)
pie.data<-as.matrix(pie.data)
legend<-rownames(pie.data)
outfile<-paste(as.vector(opt$outfile))
name<-pie.data[,1]/100
value<-as.numeric(formatC(100*name/sum(name),format ="f",digits=2))
labels<-paste(value,"%",sep="")
pieplot(x =name,legend = legend,lab.size=4,labels =labels,title =as.vector(opt$title),outfile =outfile,height = opt$height,width = opt$width,legend.pos = as.vector(opt$legend.pos),col = col,legend.ncol = opt$legend.ncol,legend.size = opt$legend.size,bg = opt$bg)

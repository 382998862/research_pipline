...
if(is.na(chrolist)||!file.exists(chrolist)){
  ...
  if (nchro>0) {
    ...
    if(opt$window<1000){
      chro<-tapply(reads[,"position"],reads[,"chromosome"],length)
      chro<-names(chro)[order(-chro)]     
    }
...
p <- p + theme(strip.text.y=element_text(angle=360, vjust=0.5), 
               ...
               #panel.grid.major = element_blank(), panel.grid.minor = element_blank()) ## set other theme opts 
...
if(!is.na(opt$col)&&opt$col!=""){
  ...
  p<-p+theme(axis.line = element_line(colour = "black"),
            ...
            #panel.border = element_blank(),
             #panel.background = element_rect(fill="#E3E3E3", colour="#E3E3E3"))
...
p<-p+theme(legend.key = element_blank(), strip.background = element_rect(colour="red", fill="#CCCCFF"))
## output to png file 
png(filename=opt$output, height = opt$height, width = opt$width, res = 500, units = "px")
print(p)
dev.off()
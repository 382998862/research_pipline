#!/share/nas2/genome/biosoft/R/3.1.1/lib64/R/bin/Rscript


# usage function
usage <- function() {
	print("-------------------------------------------------------------------------------")
	print("Usage: Rscript count_fpkm_cor.r in.txt out_prefix")
	print("1) read_count.txt: the read count file for RNA-SEQ")
	print("2) pair.txt: the pairs of sample id for cor analysis")
	print("3) out_prefix: the prefix of output")
	print("-------------------------------------------------------------------------------")
}


# get args
args <-commandArgs(TRUE)

# check args length
if( length(args) != 2 ) {
	print(args)
	usage()
	stop("the length of args != 2")
}


# load library
require(ggplot2)


# read data
print("read  data ...")
df <- read.table(args[1], header=TRUE,sep="\t")
head(df)
colnames(df) <- c("Sample", "x", "y")
head(df)
print("read data is over")


# plot
p <- ggplot(df, aes(x=x, y=y, group=Sample))
p <- p + geom_smooth(aes(colour = Sample),size=0.4,se=F,level=0.99,span=0.1) + labs(colour="Sample") + xlab("Relative Position in Genes(5'-3')")+ylab("Percent of Reads")
#size <- 18
#p <- p + theme(axis.title.x = element_text(face="bold", size=size),
#	axis.text.x  = element_text(face="bold", size=size),
#	axis.title.y = element_text(face="bold", size=size),
#	axis.text.y  = element_text(face="bold", size=size) )
#p <- p + theme(legend.title = element_text(face="bold", size=size),
#	legend.text = element_text(size=size) )
p <- p + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
if (length(unique(df$Sample))>20){p<-p + guides(col = guide_legend(nrow = 20))}

# output plot
png(filename=paste(args[2],"/Total.randchecks.png",sep=""), height = 3000, width = 6000, res = 500, units = "px")
print(p)
dev.off()

df$Sample=as.character(df$Sample)
  
for(i in unique(df$Sample)){
    individual<-subset(df,Sample==i)
    #col<-rep("#4169E1",nrow(individual))
    col<-"#4169E1"
    p<-ggplot(individual, aes(x=x, y=y))+geom_smooth(colour =col,size=0.4,se=F,level=0.99,span=0.1)+xlab("Relative Position in Genes(5'-3')")+ylab("Percent of Reads")
    p<-p+theme_bw() + theme(legend.position = "none", panel.grid.major = element_blank(), panel.grid.minor = element_blank())  
    
    png(filename=paste(args[2],"/",i,".randcheck.png",sep=""), height = 3000, width = 6000, res = 500, units = "px")
    print(p)
    dev.off()
}





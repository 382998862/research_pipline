...
if ($step == 5 && defined $cuffnorm) {
	...
	system "cat $od/final_track/$index.final_tracking.list $od/final_track/$index.newGene_filtered_final_tracking.list >$od/final_track/All.final_tracking.list";
	print OUT "perl $Bin/bin/list2expression.pl $od/final_track/All.final_tracking.list $cuffnorm $od \n";
	close OUT;
	system "sh $od/work_sh/Step5_cuffnorm.sh";
	$step = 6;
}

#if ($step == 6 && defined $cuffnorm) {
#	my @old_geneExp =sort (glob "$od/old_gene/*.geneExpression.xls");
#	my @new_geneExp =sort (glob "$od/new_gene/*.geneExpression.xls");
#	open OUT,">$od/work_sh/Step6_cuffnormf.sh" || die $!;
#	for (0..$#old_geneExp) {
#		$old_geneExp[$_] =~ /.*\/(.*)\.geneExpression.xls/;
#		print OUT "cat $old_geneExp[$_] $new_geneExp[$_] >$od/$1.all_geneExpression.xls\n";
#	}
#	close OUT;
#	system "sh $od/work_sh/Step6_cuffnormf.sh";
#}
...
if ($step == 6) {
	...
	foreach my $f (@tmp_exp) {
		my ($key) = basename($f) =~/(\w+)\.tmp\.geneExpression.xls$/;
		open (IN,"$f") or die $!;

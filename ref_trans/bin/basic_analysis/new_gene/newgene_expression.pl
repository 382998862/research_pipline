...
# ------------------------------------------------------------------
my ($gtf, $genome, $cuffnorm, $cufflinks, $od, $index, $step,$oneStepOnly,$singleexon);
GetOptions(
				"help|?"      =>\&USAGE,
				"gtf:s"       =>\$gtf,
				"genome:s"    =>\$genome,
...
if ($step==1) {
	...
	close OUT;
	system "sh $od/work_sh/Step1_get_NewGene_track.sh";
...
if ($step == 2) {
	...
	close OUT;
	system "sh $od/work_sh/Step2_newGene_format.sh";
	$step++ unless ($oneStepOnly) ;
}
...
if ($step == 3) {
	...
	close OUT;
	system "sh $od/work_sh/Step3_filter.newGene.sh";
}

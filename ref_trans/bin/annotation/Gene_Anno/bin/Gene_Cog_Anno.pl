...
foreach my $subfile (@subfiles) {
	...
	print OUT "$blastx -task blastx-fast -max_target_seqs 100 -evalue $Blast_e -db  $Database -query  $subfile -num_threads 2 -out  $odir/Cog_Dir/$name.Cog.blast &&\n";
}
close OUT;
...
print "perl $Bin/bin/blast_format.pl -id $odir/Cog_Dir -od $Tab_dir -shdir $sh_dir  -middir $odir/mid --cog -queue $queues\n";
`perl $Bin/bin/blast_format.pl -id $odir/Cog_Dir -od $Tab_dir -shdir $sh_dir  -middir $odir/mid --cog -queue $queues`;
print "perl $Bin/bin/cog_parser.pl $Tab_dir/$Q_name.Cog.blast.tab.best \n";

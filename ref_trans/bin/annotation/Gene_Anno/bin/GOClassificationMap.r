...
if(is.na(opt[["method"]])){
  ...
  yl[yl<1]<-1
  yl<-round(yl)
  axis(side = 4,at=c(0.1,33.4,66.7,100),lab=yl,cex=txt.size,tck=-0.02,srt=rot,adj=1)
  ...
  box(bty="n")
  axis(side = 1,at = bar-(bar[2]-bar[1])*0.5,labels =F,tck=-0.02)
  axis(side = 2,at=c(0,1,2,3),lab=c(0.1,1,10,100),cex=txt.size,tck=-0.02,las=1,xpd=T)
  text(x=bar, y=-0.02,labels=indat[,2], srt=rot, adj=1, xpd=TRUE,tcl=-0.02,cex=txt.size)
  yl<-c(0.1,1,10,100)*genes.total/100
  yl[yl<1]<-1
  yl<-round(yl)
  axis(side = 4,at=c(0,1,2,3),lab=yl,cex=txt.size,tck=-0.02,las=1,xpd=T)

...
while (<IN>) {
	...
	#if($id=~/Fish_newGene_3876/){print "key=$id length=$length\n";}
...
foreach my $file (@Anno_files) {
	...
	if ($database_key!~/GO/) {
		...
		while (<IN>) {
			...
			$anno=~s/\t+/\; /g;
			$Anno_Gene{$id}{$database_key}=$anno;
            print "$id not exist database=$database_key\n" unless (exists $Gene_length{$id});
			if ($Gene_length{$id}>=300 && $Gene_length{$id}<1000) {
				$Anno_Stat{$database_key}{300}++;
			}
			if ($Gene_length{$id}>=1000) {
...
if ($pathway_anno) {
		...
		while (<IN>) {
			...
			foreach my $gene(@gene_id){
				push @{$Anno_pathway{$gene}},"$pathway_id:$pathway";
...
foreach (sort keys %Anno_Base) {
	if (/eggNOG/) {
		...
		print OUT "\tKEGG_annotation\tKEGG_pathway";	
	} 
	else {

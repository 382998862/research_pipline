...
my %hetro_type = ('AG'=>'R', 'CT'=>'Y', 'GT'=>'K', 
	              'AC'=>'M', 'CG'=>'S', 'AT'=>'W',
				  'A' =>'A', 'C' =>'C', 'G' =>'G', 'T'=>'T');
my $pre_masked_vcf = "$out_dir/$key.pre_masked.vcf";
my $filtered_vcf = "$out_dir/$key.snp.vcf";
my $filtered_indel = "$out_dir/$key.indel.vcf";
...
sub filter
{
	...
	open (VCF_IND, ">", $filtered_indel) || die "Open $filtered_indel failed!\n";
	...
	while(<IN>)
	{
		...
		my ($chr, $pos, $id, $ref, $alt, $qual, $filter, $info, $format, $flag) = split /\t+/;
		next if ($ref =~ /[^ACGT]/ || $alt =~ /[^ACGT]/);
		my $Qual      = (split /:/, $flag)[-1];
		...
		}else
		{
			...
			#$snp_genotype=
			$snp_genotype = $hetro_type{$alleles} ;#if exists $hetro_type{$alleles};

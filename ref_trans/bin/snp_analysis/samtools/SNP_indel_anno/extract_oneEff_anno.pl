...
sub select_best_anno()
{
	...
	for my $anno (@snpeff_annos){
		...
		my ($impact, $functional_class, $coden_change, $amino_acid_change, $amino_acid_length,$gene_name, $gene_biotype, $gene_coding, $transcript_id, $exon_id,$Genotype_Number,$error_info) = split /\|/, $anno_info ;
		push @{$hanno{$impact}{$effect}}, [$impact, $functional_class, $coden_change, $amino_acid_change,$amino_acid_length, $gene_name, $gene_biotype, $gene_coding, $transcript_id, $exon_id, $Genotype_Number,$error_info] ;
	...
	if ($type eq 'HIGH' || $type eq 'MODERATE' || $type eq 'LOW'){
		for my $eff (keys %{$ahanno_info}){
			for my $aanno (@{$ahanno_info->{$eff}}){
				my ($impact, $functional_class, $coden_change, $amino_acid_change,$amino_acid_length, $gene_name, $gene_biotype, $gene_coding, $transcript_id, $exon_id, $Genotype_Number,$error_info) = @{$aanno} ;

...
my $queue = defined $opts{queue} ? $opts{queue} : "medical.q" ;
...
sub get_sample_files()
{
	...
	if ($#sam>=18) {
        ...
        for(my $i=9;$i<=$#sam;$i++){
			...
			if ($j<8 and $i!=$#sam) {
				...
				@head=map{$_+1}@head;
				my $head=join(",",@head);
				print "cut -f $head $vcffile >$vcffile.$n\n";
				`cut -f $head $vcffile >$vcffile.$n`;
				foreach my $k(@samp_col){
					print "$k $vcffile.$n\n";
					$sample_vcf{$k}=$vcffile.".".$n;
				}
				@head=(0..8);
				@samp_col=();
			}
	...
	while(<IN>){
		...
		if ($#sam>=18) {
			print SH "java -XX:ParallelGCThreads=5 -jar $gatk -T SelectVariants -R $reffile --variant $sample_vcf{$sample} -o $outfile -sn $sample -env \n" ;
		}
		else{

...
parser.add_option('-g', default='gene_fpkm_matrix.csv', help="where to output the gene count matrix [default: %default")
parser.add_option('-t', default='transcript_fpkm_matrix.csv', help="where to output the transcript count matrix [default: %default]")
...
samples = next(os.walk(opts.input))[1]
if len(samples) == 0:
  parser.print_help()
  print " "
...
for s in samples:
    ...
    print(s)    
    try:
        with open(glob.iglob(os.path.join(opts.input,s,"tringTie_asm.gtf")).next()) as f:
...
for q, s in enumerate(samples):
    ...
    try:
        with open(glob.iglob(os.path.join(opts.input,s,"tringTie_asm.gtf")).next()) as f: #grabs first .gtf file it finds inside the sample subdirectory
...
##                        transcriptList.append((g_id, t_id, int(ceil(coverage*transcript_len/read_len))))
                        ...
                        t_dict[t_id].setdefault(s, float(fpkm))
                        t_dict[t_id].setdefault("length", transcript_len)
                    t_id=RE_TRANSCRIPT_ID.search(v[len(v)-1]).group(1)
                    g_id=RE_GENE_ID.search(v[len(v)-1]).group(1)
                    fpkm=float(RE_FPKM.search(v[len(v)-1]).group(1))
                    transcript_len=0                
                if v[2]=="exon":
...
with open(opts.t, 'wb+') as  csvfile:
    my_writer=csv.DictWriter(csvfile,delimiter='\t', fieldnames=["#ID"]+samples+["length"])
    my_writer.writerow(dict((fn,fn) for fn in my_writer.fieldnames))
    for i in t_dict:
        t_dict[i]["#ID"]=i
        my_writer.writerow(t_dict[i])
with open(opts.g, 'wb+') as csvfile:
    my_writer=csv.DictWriter(csvfile, delimiter='\t', fieldnames=["#ID"]+samples)
##    my_writer.writerow([""]+samples)
##    my_writer.writerows(geneDict)
    my_writer.writerow(dict((fn,fn) for fn in my_writer.fieldnames))
    for i in geneDict:
        geneDict[i]["#ID"]=i #add gene_id to row

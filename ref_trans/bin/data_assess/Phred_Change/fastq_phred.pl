...
#Get options.
my ($help, $print_score, $detail, $print_ascii, $reads_num, $reads_start_arg, $reads_end_arg,$outdir);
my $reads_end_turn;
GetOptions(
	'help|h!' => \$help,
	'score|s!' => \$print_score,
...
sub main{
	...
	{
	my $bsname=basename($filename);
	my $average_value;
	my @FQ = ();
	my @all_ascii = ();
	my ($file_end, $phred_result) = ('','');
	my ($Q, $count, $lt_58, $gt_76) = (0, 0, 0, 0);
	...
	#print ASCII.
	if ($print_ascii){
		#print "\n","ASCII Value: $filename","\n";
		$average_value=&print_array_of_array(\@all_ascii, 0, $detail);
		#next;
	}
	...
	foreach my $ascii_ref (@all_ascii){
		...
		$gt_76 += (grep { $_ >= 76} @{$ascii_ref});
	}

	#Guess the Phred with ASCII value range. 
	if ($lt_58 > 1 && $gt_76 == 0 ){   # 有2个以上的质量字符ASCII值小于等于58，同时没有任何质量字符的ASCII值大于等于76
		$Q = 33;
		$phred_result=33;
		#$phred_result = "$filename: The encoding should be Phred33.\nThe quality score character number that ASCII value less than 58 : $lt_58\nThe quality score character number that ASCII value great than 76: $gt_76";
	}elsif($lt_58 == 0 && $gt_76 > 1){  # 有2个以上的质量字符ASCII值大于等于76，同时没有任何质量字符的ASCII值小于等于58
		$Q = 64;
		$phred_result =64;
		#$phred_result = "$filename: The encoding should be Phred64.\nThe quality score character number that ASCII value less than 58 : $lt_58\nThe quality score character number that ASCII value great than 76: $gt_76";
	}elsif($lt_58 == 0 && $gt_76 == 0){ # 所有质量字符的ASCII值介于59到75之间，即判断可能是Phred+33 
		$Q = 33;
		$phred_result=33;		
		...
		#exit;
	}elsif($average_value<77){ # 以上情况之外的话，根据ASCII值的平均值判断，若小于77，则可能是Phred+33
		$Q = 33;
		$phred_result=33;	
		#print STDERR "$filename\nWarning: Abnormal encoding, Please test again with more reads or make a judgement by yourself with ASCII value by '-ascii' options.\n"; 
		#exit;
	}
	...
	open (OUT,">$outdir/$bsname.phred") or die "Can not open $filename:$!\n";
	#Print the phred encoding result.
	print OUT "$phred_result";
	close OUT;
	}
}

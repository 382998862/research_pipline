...
my ($od,$conf,$count,$fpkm,$target,$cis_target,$trans_target,$anno,$type,$step,$ppi,$newgene_gff);
GetOptions(
        "h|?"           =>\&USAGE,
	"od:s"     	=>\$od,
	"count:s"	=>\$count,
...
&run_or_die("$Rscript $Bin/cor/fpkm_cor.r $fpkm $od/density");
...
=head
my @seps=&getSep($conf);
if(defined $bamdir && defined $gtf && scalar(@seps)>0){
	my @bams=glob("$bamdir/*/*.bam");
	mkdir "$od/htseq_count"	unless(-d "$od/htseq_count");
	open CONFIG,">$od/htseq_count/bam_to_exon_count.config" ||die $!;
	foreach my $bam (@bams) {
		print CONFIG "$_\n";
	}
	close CONFIG;
        $cmd="$Rscript $Bin/DEU/bam_count.R  infile=$od/htseq_count/bam_to_exon_count.config gtf=$gtf type=bam od=$od/htseq_count queue=$queue";
        &run_or_die($cmd) unless (-e "$od/htseq_count/path.txt");
...
&run_or_die("perl $Bin/bin/gene_counts_to_FDR_FC.pl -count $count -conf $conf -fpkm $fpkm -type $type -filter $filter -od $od");
...
if(@degs>1 && @degs<=5){
	my $cmd="$Rscript $Bin/vn.r ".join(",",@degs)." $od/All_DEG  Veen";
	`$cmd`;
	&run_or_die($cmd);		
}
...
if(!defined $target && !defined $cis_target && !defined $trans_target){
	...
	foreach my $deg(@degs){
		my $num=`wc -l $deg`;	chomp($num);
		$num=(split(/\s+/,$_))[0];
		my $base=basename $deg;my $vs=(split(/\./,$base))[0];
		my $odir="$od/$vs/Anno_enrichment"; `mkdir $odir`   unless(-d $odir);
		my $cmd = "$rscript $Bin/enrich/enrich.r --all $od/$vs/$vs.all --deg $deg --go $od/GO.info --kegg $od/KEGG.info --prefix $vs --od $odir/enrich ";
		$cmd .="&& perl $Bin/enrich/bin/plot_deg_KEGG.pl $odir/enrich/${vs}_KEGG_pathway_enrich.list $deg $odir/enrich ";
		$cmd .="&& perl $Bin/enrich/anno.pl -input $deg -od $odir/anno -anno $anno ";
		$cmd .="&& perl $Bin/enrich/anno/Kegg_map_web.pl -d $deg -k $vs -i $anno -o $odir ";
		$cmd .="&& perl $Bin/enrich/anno/cosmic_TF_anno/medical_anno.pl -deg $deg -o $od/$vs -db $config{medical} "	if(exists $config{medical});		
		print SH "$cmd\n";
...
Usage:
	...
	Options:
	-h	Help
	-conf	file	forced		config file contained Com/Sep et. al info
